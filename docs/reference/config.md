...
data:
  ...
  zones:                     # exactly two runtime zones in the region
    - europe-west1-b
    - europe-west1-c
...
| `zones` | both | yes | **exactly two** zones in `region` (subnets/nodeLocations) |
...
| `gcpNodesPerZone` | GCP | no | runtime nodes per zone (default `3` → 6 nodes across two zones) |
